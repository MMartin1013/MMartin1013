# Hi, I'm Manolo Martin 👋

Product-Minded Full-Stack Engineer & Startup Co-Founder based in Washington DC-Baltimore Area

- 🚀 **Currently:** Full-Stack Software Engineer at Pluralsight & building mobile/AI apps
- 🛠️ **Check out my portfolio**: [Here](https://portfolio.manolo-martin.workers.dev/)
- 📫 **Contact:** manolo.martin.dev@gmail.com | [LinkedIn](https://linkedin.com/in/manolomartin)

---

### 💻 Tech Stack

**Languages & Frameworks:**  
`TypeScript` `JavaScript` `C#` `Python` `Dart` `Java` `React` `Next.js` `Svelte` `Express.js` `ASP.NET` `Flutter`

**AI & Cloud Infrastructure:**  
`LangChain` `ChromaDB` `Ollama` `MCP` `AWS EKS` `Azure AKS` `Terraform` `Cloudflare` `Docker` `PostgreSQL`

**Observability & Tooling:**  
`Sentry` `New Relic` `Grafana` `LaunchDarkly` `GitHub Actions` `PostHog`

<!---
MMartin1013/MMartin1013 is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
